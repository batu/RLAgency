...
class ConvDictWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        self.occupancy_size = (1, 9, 5, 9)

        self.observation_space = gym.spaces.Dict(
            spaces={
                "vec": gym.spaces.Box(-1, 1, (self.vector_size,)),
                "depthmap": gym.spaces.Box(0, 1, self.img_size, dtype=np.uint8),
                "occupancy": gym.spaces.Box(0, 1, self.occupancy_size, dtype=np.uint8),
            }
        )

    def preprocess_np(self, input_array):
    ...
    def step(self, action: Union[np.ndarray, int]):
        ...
        obs_dict = {
            "vec":vector_obs,
            "depthmap":depth_obs2d,
            "occupancy": occupancy_obs3d
...
class UnityToGymMultipleWrapper(gym.Env):
    """
